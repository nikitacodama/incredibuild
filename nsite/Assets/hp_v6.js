...
/**
 * Fade <li> by distance to viewport center + shrink each block's divider.
 * Works for ALL .hpblock groups on the page.
 */
function initScrollFadeMulti() {
  const SELECTORS = {
    block: ".hpblock",         // each section container
    list: "ol",                // list inside the block
    dividerSpan: ".divider > span"
  };
  const BASE_OPACITY = 0.3;
  const MAX_OPACITY  = 1.0;
  const FALLOFF      = 0.35;   // fraction of viewport height

  // Collect all eligible blocks
  const blocks = Array.from(document.querySelectorAll(SELECTORS.block))
    .map(block => {
      const list = block.querySelector(SELECTORS.list);
      if (!list) return null;
      const items = Array.from(list.querySelectorAll("li"));
      const dividerSpan = block.querySelector(SELECTORS.dividerSpan);
      if (dividerSpan) dividerSpan.style.transformOrigin = "bottom center";
      return { block, list, items, dividerSpan };
    })
    .filter(Boolean);
  ...
  function updateAll() {
    ...
    for (const { list, items, dividerSpan } of blocks) {
      // 1) Fade each <li> by distance to center
